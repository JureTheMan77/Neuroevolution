...
for (dokler nismo zavodoljni z agenti populacije) {
    ...
    \begin{itemize}
        ...
        \item vzorčenje (razdelek~\ref{sec:vzorcenje}) z algoritmom stohastičnega univerzalnega vzorčenja,
    ...
    Vsak agent hrani informacijo o svoji kvaliteti, točnosti in MCC (razdelek~\ref{subsec:matthewsov-korelacijski-koeficient}), ki se ob inicializaciji populacije (razdelek~\ref{sec:inicializacija-populacije})
    ...
    \begin{description}
        ...
        \item[Metrika kvalitete]{Metrika, po kateri želimo izvajati vzorčenje. Lahko je točnost ali MCC (razdelek~\ref{subsec:matthewsov-korelacijski-koeficient}).}
    \end{description}
...
while (seznam_izbranih.size() < agentsToKeep) {
    ...
    Proces propagira vse zapise učne množice skozi vse agente populacije, preveri pravilnost napovedi in
    izpolni matriko zmot.
