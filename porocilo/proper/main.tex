...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Rezultate smo ovrednotili in primerjali s pristopom \enquote{random forest} (poglavje~\ref{ch:rezultati}).
    ...
    \begin{itemize}
        ...
        \item podatkovna množica Shuttle (podrazdelek~\ref{subsec:statlog-shuttle}) s 58000 vrsticami, od katerih okoli 80\% pripada enemu razredu,
    \end{itemize}

    \subsection{Iris}\label{subsec:iris}
    Ta podatkovna množica~\cite{misc_iris_53} je najenostavnejša izmed izbranih.
    Vsebuje po 150 učnih primerov s štirimi atributi, ki predstavljajo širino in dolžino (v centimetrih) cvetnih in čašnih
    ...
    Prvi razred ima 59 učnih primerov (33.15\%), drugi 71 (39.89\%), tretji pa 48 (26.97\%).
    ...
    \begin{description}
        \item[nesprejemljivo]{1210 primerov, 70.02\%,}
        \item[sprejemljivo]{384 primerov, 22.22\%,}
        \item[dobro]{69 primerov, 3.99\% in}
        \item[zelo dobro]{65 primerov, 3.76\%.}
    \end{description}
    ...
    \begin{center}
        \begin{tabular}{||l c c||}
            \hline
            atribut                 & diskretne vrednosti   & številske vrednosti \\ \hline
    ...
    Testiranje smo izvedli še s pristopom \enquote{random forest} s programom Orange~\cite{JMLR:demsar13a} po naslednjem postopku:
    \begin{enumerate}
        ...
        \item na koncu smo primerjali točnost in MCC z našim pristopom.
    \end{enumerate}
    ...
    Pristop \enquote{random forest} je dosegel točnost $100\%$ (tabela~\ref{tab:rforest_iris_result}).
    ...
    Pristop \enquote{random forest} je dosegel enako točnost $96.2\%$ (tabela~\ref{tab:rforest_wine_result}).
    ...
    Pristop \enquote{random forest} je dosegel precej večjo točnost $95.9\%$ s 300 drevesi (tabela~\ref{tab:rforest_car_result}).
    ...
    Pristop \enquote{random forest} je dosegel točnost $100\%$ (tabela~\ref{tab:rforest_shuttle_result}).


    \chapter{Zaključki, sklep}\label{ch:zakljucki-sklep}
    V diplomski nalogi smo napisali program, ki ustvari rekurenčne nevronske mreže z genetskim algoritmom in primerjali
    njihovo točnost s obstoječim pristopom \enquote{random forest}.
    Za ta namen smo v teoretičnem delu predstavili teoretične koncepte nevronskih mrež, genetskih algoritmov in
    nevroevolucije ter predstavili obstoječ pristop \textbf{TODO}.


    Iz rezultatov sklepamo, da naš izračun kvalitete ni zadosten za obdržanje nišnih lastnosti.

    \section{Izboljšave}
    bias na vseh globokih vozliščih na 1

    boljši izračun kvalitete
