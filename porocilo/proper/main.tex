...
for (dokler nismo zavodoljni z agenti populacije) {
    ...
    Struktura grafov je pogojena izbrani podatkovni množici.
    ...
    Vrednost se nastavi v koraku~\ref{sec:izracun-kakovosti}, vsebuje pa rezultat aktivacijske funkcije.
    ...
    \begin{itemize}
        ...
        \item preko katerih ne moremo doseči nobenih izhodnih vozlišč
    \end{itemize}
    Mirujoča vozlišča lahko postanejo aktivna, če je izbrana prava mutacija v koraku~\ref{sec:mutiranje}.
    ...
    \begin{itemize}
        ...
        \item indeks, ki je unikaten glede na celoten graf (določi se na enak način kot za vozlišča v~\ref{subsec:vozlisca})
        \item utež, ki se v koraku inicializacije populacije (\ref{sec:inicializacija-populacije}) in mutiranja (\ref{sec:mutiranje}) normalizira med -1 in 1
    ...
    \begin{description}
        ...
        znotraj posameznega agenta. Korak mutiranja (\ref{sec:mutiranje}) lahko povzroči izgubo ali pridobitev nove povezave,
        ...
        agentov na konzolo in pri shranjevanju grafov agentov v JSON datoteke. }
    \end{description}
    ...
    \begin{enumerate}
        ...
        morajo slediti pravilom poglavja~\ref{subsec:povezave}.
    ...
    \begin{description}
        ...
        \item[Metrika kvalitete]{Metrika, po kateri želimo izvajati vzorčenje. Lahko je točnost ali MCC (\ref{subsec:matthewsov-korelacijski-koeficient}).}
    \end{description}
    ...
    \begin{enumerate}
        \item Poišče agenta z največjo kvaliteto, izračunano v koraku izračuna kakovosti (\ref{sec:izracun-kakovosti}), ki
...
while (seznam_izbranih.size() < agentsToKeep) {
    ...
    \begin{figure}[htb]
        ...
        \end{center}
        \caption{Ilustracija SUS, kjer želimo obdržati 3 agente. Izbrani agenti bodo A, B in D. Če bi bil začetni odmik
    ...
    Korak križanja agentov napolni podmnožico populacije iz koraka vzorčenja (\ref{sec:vzorcenje}) s potomci.
    ...
    \begin{description}
        ...
        izbrano naključno po pravilih v poglavju~\ref{subsec:povezave}. }
    ...
    Mutiranje vsakega potomca izvajamo v svoji procesorski niti.

    Po končanem postopku mutiranja vsakemu potomcu normaliziramo uteži v svoji procesorski niti.


    \section{Izračun kakovosti}\label{sec:izracun-kakovosti}
    \textbf{TODO}

    \subsection{Matthewsov korelacijski koeficient}\label{subsec:matthewsov-korelacijski-koeficient}
    \textbf{TODO}
